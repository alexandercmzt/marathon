...
def getFinishTime(finishTime, raceName):
    try:
...
for participantNumber, raceList in participants.items():
    ...
    for race in raceList:
        if "2012" in race[0]:
            ...
            year2015 += 1
            if "Marathon Oasis Rock 'n' Roll de Montreal" in race[1].lower() and "demi" not in race[2].lower:
                raceName = race[1]
                attendedMontreal2015 = True
                montreal2015FinishTime = getFinishTime(race[3], raceName)
                if training: #if this is the training set their time is the average time.
                    year2015 -= 1
                    distanceFromAverage = marathonAverageTime[raceName]
                    continue                    
        elif "2016" in race[0]:
            year2016 += 1       
        ...
        if "marathon" in race[1].lower():
            numberOfMarathons += 1
            if "montreal" in race[1].lower():
                numberOfMontrealMarathons += 1
            if any(word in race[2].lower() for word in ["demi","half"]):
                raceName = race[1] + " demi"
            else:
                raceName = race[1]
            finishSeconds = getFinishTime(race[3], raceName) 
            distanceFromAverage += finishSeconds - marathonAverageTime[raceName]

    ###NOW RETURN ARRAYS### 
    distanceFromAverageAverage = distanceFromAverage/numberOfMarathons
    if totalNumberOfAgedRaces != 0:
        age = age/totalNumberOfAgedRaces 
